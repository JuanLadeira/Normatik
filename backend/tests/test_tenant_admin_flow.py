...
async def test_admin_tenant_lifecycle(client, admin_token, db_session):
    ...
    response = await client.post(
        "/api/admin/tenants/", json=tenant_data, headers=headers
    )
    ...
    response = await client.patch(
        f"/api/admin/tenants/{tenant_id}", json=update_data, headers=headers
    )
    assert response.status_code == 200
    ...
    # 5. Suspender Tenant
    response = await client.post(
        f"/api/admin/tenants/{tenant_id}/suspend", headers=headers
    )
    assert response.status_code == 200
    ...
    response = await client.post(
        f"/api/admin/tenants/{tenant_id}/activate", json=activate_data, headers=headers
    )
    assert response.status_code == 200
